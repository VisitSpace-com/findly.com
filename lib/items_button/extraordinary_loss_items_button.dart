...
import '../items/extraordianry_loss_items.dart';

class ExtraordinaryLossItemsButton extends ConsumerStatefulWidget {
  const ExtraordinaryLossItemsButton({super.key});

  @override
  ConsumerState<ExtraordinaryLossItemsButton> createState() =>
      OperatingCostItemsButtonState();
}

class OperatingCostItemsButtonState
    extends ConsumerState<ExtraordinaryLossItemsButton> {
  bool isHovered = false;

  @override
  Widget build(BuildContext context) {
    final selectedItems = ref.watch(extraordinaryLossItemsProvider);
    final notifier = ref.read(extraordinaryLossItemsProvider.notifier);
      ...
      children: [
        Expanded(
          child: GridView.builder(
            ...
            itemCount:
                extraordinaryLossItems.length + 1, // リストの項目数 + 1 for text field
            itemBuilder: (context, index) {
              // 最後のアイテムをテキストフィールドにする
              if (index == extraordinaryLossItems.length) {
                ...
                String itemKey = extraordinaryLossItems.keys.elementAt(index);
                ...
                return SizedBox(
                  ...
                  child: MouseRegion(
                    ...
                    child: Tooltip(
                      verticalOffset: 100,
                      message: extraordinaryLossItems[itemKey],
